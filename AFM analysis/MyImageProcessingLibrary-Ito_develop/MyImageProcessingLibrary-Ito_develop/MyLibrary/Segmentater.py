...
class Segmentater:
    def __init__(
        self,
        area_min=200,
        area_min_connecting=200,
        apply_no_connecting = True,
        low_threshold=1.5,
    ...
    def __call__(self, image):
        ...
        no_small_binary_image4 = binary_closing(self.no_low_binary_image)
    ...
    def _remove_small_fragments(binary_image, area_min):
    ...
    def _remove_nonlinear_objects(self, binary_image, h_length, h_sratio, linegap=1):
        out_binary_image = binary_image.copy()
        n_labels, label_image, stats, centers = cv2.connectedComponentsWithStats(
            np.uint8(out_binary_image), 8
        )
        for i in range(1, n_labels):
            try:
                ...
                hough_lines = probabilistic_hough_line(
                    target_edge, line_length=h_length, line_gap=linegap
                )
                line_lengths = [
                    np.linalg.norm(np.array(start) - np.array(end)) for (start, end) in hough_lines
                ]  # あってるか自信ない
                total_length = sum(line_lengths)
                s_ratio = total_length / np.sum(target_edge)
                self.h_sratio_list.append(s_ratio)

                if s_ratio < h_sratio and np.sum(target) < 1000:  # keep big object larger than 500
    ...
    def _remove_connecting_fragments(self, binary_image):
        out_binary_image = binary_image.copy()
        ...
        out_binary_image = binary_closing(out_binary_image)
        return out_binary_image
    ...
    def remove_low_component(self, height_image, binary_image):
        ...
        for i in range(1, n_labels):
            ...
            if max_height < self.low_threshold:
                out_binary_image[np.where(label_image == i)] = 0
        return out_binary_image
