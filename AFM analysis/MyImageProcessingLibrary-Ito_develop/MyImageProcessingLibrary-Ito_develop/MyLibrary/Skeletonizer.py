...
class Skeletonizer:
    def __init__(self, bp_height=5, branch_length=8, min_area=10):
    ...
    def __call__(self, image: ProcessedImage):
        ...
        # save each segments data to ProcessedImage instance
        ...
        image.ep = imptools.endPoints(nosmall_skeleton_image)
        image.bp = imptools.branchedPoints(nosmall_skeleton_image)


    def prune_branches(self, calibrated_image, init_skeleton_image):
        branches_image = self.calc_branches_image(calibrated_image, init_skeleton_image)
        return init_skeleton_image - branches_image

    def calc_branches_image(self, calibrated_image, init_skeleton_image):
    ...
    def set_low_bp_coor(self, calibrated_image, init_skeleton_image, bp_height):
    ...
    def get_close_eps(self):  # eps eny close than branch_length
        ...
        for x, y in zip(all_eps_coorx, all_eps_coory):
            window = _low_bps_image[
                x - self.branch_length : x + self.branch_length,
    ...
    def track_branches(self):
        ...
        for step_num, (start_x, start_y) in enumerate(zip(starts_x, starts_y)):
            ...
            tracking_area = image_for_tracking[
                start_x - self.branch_length : start_x + self.branch_length,
            ...
            xtrack = [
                x + start_x - self.branch_length
            ]  # 各枝ごとの結果格納用(初期位置であるtracking_areaの中心は最初に追加)
            ...
            try:
                for i in range(self.branch_length):
                    ...
                    if (window == 0).all():  # windowsの中身が全て0(つまり別のepにたどり着いた)なら停止.極端に短いCNFを想定している？
                        ...
                        break
                    elif image_for_low_bp_detection[
                        x - 1 : x + 2, y - 1 : y + 2
                    ].any():  # low_branchの手前になったら消去。
                        branches_coor_x += xtrack
                        branches_coor_y += ytrack
                        break
    ...
    def remove_small_and_ring(self, skeleton_image):
        ...
        ep = imptools.endPoints(returned_image)
        ring_frac_label = np.setdiff1d(np.arange(1, nLabels), (ep * label_Images))
        for i in range(1, nLabels):
            area = data[i][4]
            if area < self.min_area:
                returned_image[np.where(label_Images == i)] = 0
