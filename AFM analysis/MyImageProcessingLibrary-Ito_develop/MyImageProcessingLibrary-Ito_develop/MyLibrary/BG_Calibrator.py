import sys
sys.path.insert(0, '/Users/tomok/PycharmProjects/MyImageProcessingLibrary/MyLibrary')

from typing import Optional

import cv2
import numpy as np
from scipy import stats
from scipy.interpolate import interp1d
from scipy.signal import savgol_filter

from .ProcessedImageClass import ProcessedImage


class BG_Calibrator:
    def __init__(self, savgol_window=31, savgol_polyorder=2, apply_median=True):
        """

        :param savgol_window: int
        :param savgol_polyorder: int
        :param apply_median: boolean
        """

        self.savgol_window = savgol_window
        self.savgol_polyorder = savgol_polyorder
        self.apply_median = apply_median
        self.threshold_low: Optional[np.ndarray] = None
        self.threshold_high: Optional[np.ndarray] = None
        self.background_filter: Optional[np.ndarray] = None
        self.background_smoothed = None

    def __call__(self, image: ProcessedImage):
        """

        :param image: ProcessedImage class instance

        :return: Image class instance with attribution of calibrated_image
        """

        self.threshold_low, self.threshold_high = self._get_thresholds(image.original_image)

        # 以下のfilterは、ただ中間結果を保持しているだけなので、改善できる気がする
        self.background_filter = self._get_background_filter(
            image.original_image, self.threshold_low, self.threshold_high
        )

        self.background_smoothed = self._savgol_smoothed_background(image)

        calibrated_image = image.original_image - self.background_smoothed
        if self.apply_median:
            calibrated_image = cv2.medianBlur(calibrated_image.astype(np.float32), ksize=3)

        image.calibrated_image = calibrated_image

    @staticmethod
    def _get_thresholds(image: np.ndarray):  # 各行の外れ値の閾値を取得（2次元array）
        """

        :param image: ndarray shape=(ImageSize, ImageSize)

        :return:
        """
        weight_q1 = stats.scoreatpercentile(image, 25, axis=1).reshape((image.shape[0], 1))
        weight_q3 = stats.scoreatpercentile(image, 75, axis=1).reshape((image.shape[0], 1))
        weight_iqr = weight_q3 - weight_q1
        thresholds_low = weight_q1 - weight_iqr * 1.5
        thresholds_high = weight_q3 + weight_iqr * 0
        return thresholds_low, thresholds_high

    @staticmethod
    def _get_background_filter(
        image: np.ndarray, thresholds_low: np.ndarray, thresholds_high: np.ndarray
    ):

        """
        get the filter to remove the outlier pixel(CNF and depression) from AFM image

        :param image: ndim shape = (ImageSize, ImageSize)
        :param thresholds_low: ndim shape = (ImageSize, 1)
        :param thresholds_high: ndim shape = (ImageSize, 1)

        :return: 2D-ndarray[bool] that represents background_pixel position.
        """
        background_filter = (image > thresholds_low) & (image < thresholds_high)
        background_filter[:, 0] = True
        background_filter[:, -1] = True
        return background_filter

    def _savgol_smoothed_background(self, image: ProcessedImage):
        n_row, n_col = image.original_image.shape
        splined_bg = np.empty_like(image.original_image)

        for n, (height_row, filter_row) in enumerate(
            zip(image.original_image, self.background_filter)
        ):
            smoothed_bg_row = savgol_filter(
                height_row[filter_row], self.savgol_window, self.savgol_polyorder
            )
            splined_bg_row = interp1d(np.where(filter_row)[0], smoothed_bg_row)(np.arange(n_col))
            splined_bg[n] = splined_bg_row
        return splined_bg
