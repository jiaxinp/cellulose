...
def track_contour(fiber, save_dir, save_fig):  # todo
    if save_fig:
        ...
        ax.plot(fiber.horizon, fiber.height)
        ax.set_xlim(-50, 900)
        ax.set_ylim(0, 4)
        ax.set_xticks(np.arange(0, 1000, 200))
        ax.set_yticks(np.arange(0, 5, 1))
        for i in fiber.kink_indices:
            ax.axvline(x=fiber.horizon[i], c="red", ls="--", lw=0.75)
...
def save_fiber_AFM(fiber, save_dir, save_fig):
    if save_fig:
        fig, ax = plt.subplots(1, 1, figsize=(3, 1.5), dpi=900)
        ax.axis("off")
        ax.imshow(fiber.AFM_image, cmap="afmhot")
        ax.scatter(
            fiber.ytrack[fiber.kink_indices],
...
if __name__ == "__main__":
    ...
    for each_image_dir in fiber_data_dir.iterdir():
        ...
        with open(save_dir / "average_length_data.txt", "w") as result:

            for n, f in enumerate(each_image_dir.glob("*.pickle")):
                ...
                result.write(f"{f.stem}: {np.average(fiber.height)}\n")
