...
def calc_categ_length(
    image_lst: List[ProcessedImage], category: str
...
def genr_categ_skel(
    img: ProcessedImage, category: str, height_thresh: tuple = (2.02, 3.78)
) -> np.array:
    """
...
def genr_dent_categ_skel(
    img: ProcessedImage, dent_category: str, height_thresh: tuple = (2.02, 3.78)
) -> np.array:
    """
    :param img:
...
if __name__ == "__main__":
    data_name_dict = {
        n: file.stem for n, file in enumerate(data_name_dir.iterdir()) if file.is_dir()
    }
    ...
    if not dent_categ_image_dir.exists():
        dent_categ_image_dir.mkdir(parents=True)
    ...
    for file in image_objects_path:
        ...
        #  todo 一つ上の行の変数を作らないと画像保存できない仕様が嫌い。imageを渡すだけで動くようにしたい。将来的に改善
        save_dent_category_image(
            image,
            dent_label_image,
            categoly_label_dict,
