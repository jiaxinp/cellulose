...
class DentAnalyzer:
    ...
    # 時間が無いので諦める。
    def __init__(
        self, upper_thresh=3.78, lower_thresh=2.02  # todo 現状イニシャライザに意味はない。将来的にここから値を参照する様に改善する
    ):
    ...
    def calc_breakdown(self, image_list: List[ProcessedImage]) -> BreakdownDict:
    ...
    def calc_dl(self, image_list: List[ProcessedImage]) -> Dent_DepthLength_Dict:
        ...
        for category in dent_categories:
            dl_dict[category] = (
                list(self._calc_dent_depth(image_list, category)),
                list(self._calc_dent_length(image_list, category)),
            )
        return dl_dict

    @staticmethod
    def _calc_dent_depth(image_list: List[ProcessedImage], category: str) -> Generator:
        for img in image_list:
            for label in img.categorized_dent_labeldict[category]:
                depth = 2.02 - np.min(
                    img.calibrated_image[np.where(img.dent_skel_labelimg == label)]
                )
                yield depth

    @staticmethod
    def _calc_dent_length(image_list: List[ProcessedImage], category: str) -> Generator:
        for img in image_list:
            for label in img.categorized_dent_labeldict[category]:
    ...
    @staticmethod
    def save_dent_category_image(
        img: ProcessedImage, dent_label_img: np.array, categ_label_dict: dict, savename: Path
    ):
        ...
        for category, labels in categ_label_dict.items():
            ...
            ax.scatter(categ_posi[1], categ_posi[0], s=10, c=category_colors[category])
        plt.savefig(savename, dpi=900)

    def _calc_categ_length(
        self, image_lst: List[ProcessedImage], category: str
    ...
    def _genr_categ_skel(
        self, img: ProcessedImage, category: str, height_thresh: tuple = (2.02, 3.78)
    ) -> np.array:
        """
        ...
        if category in {"kink", "ep", "straight", "kinked_end"}:
            ...
            dent_skel = img.skeleton_image & (
                img.calibrated_image < height_thresh[0]
            )  # 下限値0.3は不要？
            return dent_skel
            ...
            normal_skel = (
                img.skeleton_image
                & (height_thresh[0] <= img.calibrated_image)
    ...
    def _genr_dent_categ_skel(self, img: ProcessedImage, dent_category: str) -> np.array:
        """
        :param img:
        ...
        d_categ_skel = np.where(np.isin(img.dent_skel_labelimg, category_label), 1, 0).astype(
            np.uint8
        )
        return d_categ_skel

    @staticmethod
    def _calc_dent_categ_dict(
        img: ProcessedImage, height_thresh: tuple = (2.02, 3.78)
    ) -> Tuple[np.array, Dict]:
        """
        :param img:
        :param dent_category: Category of dents. This value should be set to one of 'kink','ep','straight','kinked_end'
        ...
        # todo skeleton_imageはbpを除去しなくても良い？
        # todo size=1のやつは除去しなくて良い？
        dent_skel = (img.skeleton_image & (img.calibrated_image < height_thresh[0])).astype(
            np.uint8
        )
        ...
        category_label_dict = {
            "kink": np.setdiff1d(all_kink_label, all_ep_label),
